...
- **Logic**: Matches based on `National ID` uniqueness and `Salary Difference <= office.salary_match_limit` (configured by manager, default 50 LYD, max 50 LYD). **Workplace constraint is exempted (v1.7.0)**, allowing different employers to match automatically.
- **Database Functions**: `find_potential_guarantors()` with `override_validation` parameter for Monitor role (dynamically reads and enforces office limits). `attempt_auto_match()` for automated linking.
- **Dynamic Redirection Wizard (v1.7.0)**: Upon successful matchmaking, a premium **"اعتماد الحساب المالي الجديد 📊"** button appears, guiding the user to the Calculator (running the Lowest Salary Rule reactively) and dynamically routing the user on save depending on their role (Staff to Dashboard with prompt; Manager/Accountant to Settlements).
...
- **Phase 35 (Cyber Safeguard Telemetry & Database Schema Hardening - v1.5.1)**: Complete ✅ — Secured `public.offices` SELECT policy via JWT tenant isolation, preventing registration join code leaks. Fixed `CustomerList.tsx` clipboard copying handler to respect phone masking configurations. Aligned database schemas by adding `market_sale_price` and settlement columns natively, and integrated `onFocus` clear-on-focus UX in forms.
- **Phase 36 (Applicant Identity Tracking Upgrade - v1.5.2)**: Complete ✅ — Implemented dynamic applicant employee name logging for financial requests. Pre-populated the human user name in input fields but left editable. Persisted name in `created_by_name` on table `financial_requests` and updated the manager review board.
- **Phase 37 (Lead Conversion Tracking & Security Audit Logging - v1.6.0)**: Complete ✅ — Implemented potential-to-active customer conversion tracking with automatic localStorage backup and green status indicators. Created centralized security telemetry logs (`audit_logs` table) recording deletions, updates, queue submissions, and clipboard data copies of sensitive fields (NID, Phone, Transaction ID). Integrated conversion and lead generation rates into the staff statistics dashboard (`StaffStats.tsx`) showing performance analytics per employee.
- **Phase 38 (Workplace Matchmaking Exemption & Dynamic Redirection - v1.7.0)**: Complete ✅ — Removed the same-workplace restriction from `find_potential_guarantors()`, allowing cross-employer automatic matchmaking. Integrated a visual redirection wizard flow in `WaitingQueue.tsx` with a premium action button guiding matched transactions directly to the Calculator running the Lowest Salary Rule reactively. Implemented role-based post-save redirection routing in `OfficeLayout.tsx` (Staff to Dashboard with confirmation prompt; Manager/Accountant dynamically to Settlements via `quick_settle_tx_id` cache). Extended `FinancialRequest.tsx` to allow Office Managers to submit financial requests in addition to reviewing and approving/rejecting them.
